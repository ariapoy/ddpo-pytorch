...
@torch.no_grad()
def pipeline_with_logprob_with_noisyimg(
    self: StableDiffusionPipeline,
    prompt: Union[str, List[str]] = None,
    height: Optional[int] = None,
    width: Optional[int] = None,
    ...
    with self.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            # get noisy images
            if not output_type == "latent":
                noisy_image = self.vae.decode(
                    latents / self.vae.config.scaling_factor, return_dict=False
                )[0]
                noisy_image, has_nsfw_concept = self.run_safety_checker(
                    noisy_image, device, prompt_embeds.dtype
                )
            else:
                noisy_image = latents
                has_nsfw_concept = None
            ...
            if has_nsfw_concept is None:
                do_denormalize = [True] * noisy_image.shape[0]
            else:
                do_denormalize = [not has_nsfw for has_nsfw in has_nsfw_concept]

            noisy_image = self.image_processor.postprocess(
                noisy_image, output_type=output_type, do_denormalize=do_denormalize
            )

            all_latents.append(latents)
            all_log_probs.append(log_prob)
            # store noisy images
            all_noisy_imgs.append(noisy_image)
    ...
    return image, has_nsfw_concept, all_latents, all_log_probs, all_noisy_imgs